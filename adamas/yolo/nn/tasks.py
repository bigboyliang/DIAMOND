...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, 
                          ...
                          Detect_LSDECD, Segment_LSDECD, Pose_LSDECD)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="YOLOv8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, 
                          ...
                          Detect_LADH, Segment_LADH, Pose_LADH, OBB_LADH, Detect_LSCSBD, Segment_LSCSBD, Pose_LSCSBD, OBB_LSCSBD, Detect_NMSFree, Detect_LSDECD,
                          Segment_LSDECD, Pose_LSDECD, OBB_LSDECD)):
            s = 640  # 2x min stride
            ...
            if isinstance(m, (DetectAux,)):
                ...
                forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Segment_Efficient, Segment_LSCD, Segment_TADDH, Pose, Pose_LSCD, Pose_TADDH, OBB, OBB_LSCD, OBB_TADDH, 
                                                                         Segment_LADH, Pose_LADH, OBB_LADH, Segment_LSCSBD, Pose_LSCSBD, OBB_LSCSBD, Segment_LSDECD, Pose_LSDECD, OBB_LSDECD)) else self.forward(x)
            try:
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        ...
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, 
                 ...
                 Detect_LSDECD, Segment_LSDECD, Pose_LSDECD, OBB_LSDECD):
            m.inplace = inplace
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    for m in model.modules():
        ...
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_DyHead, 
                 ...
                 Detect_LSCSBD, Segment_LSCSBD, Pose_LSCSBD, OBB_LSCSBD, Detect_NMSFree, Detect_LSDECD, Segment_LSDECD, Pose_LSDECD, OBB_LSDECD):
            m.inplace = inplace
        elif t is nn.Upsample and not hasattr(m, "recompute_scale_factor"):
            m.recompute_scale_factor = None  # torch 1.11.0 compatibility
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Faster, C2f_ODConv, C2f_Faster_EMA, C2f_DBB,
                     ...
                     C3_KAN, C2f_KAN, C3_EIEM, C2f_EIEM, C3_DEConv, C2f_DEConv, C3_SMPCGLU, C2f_SMPCGLU):
            ...
            c2 = sum(ch[x] for x in f)
                   ...
                   Pose, Pose_LSCD, Pose_TADDH, OBB, OBB_LSCD, OBB_TADDH, Detect_LADH, Segment_LADH, Pose_LADH, OBB_LADH,
                   Detect_LSCSBD, Segment_LSCSBD, Pose_LSCSBD, OBB_LSCSBD, Detect_NMSFree, Detect_LSDECD,
                   Segment_LSDECD, Pose_LSDECD, OBB_LSDECD):
            args.append([ch[x] for x in f])
            if m in (Segment, Segment_Efficient, Segment_LSCD, Segment_TADDH, Segment_LADH, Segment_LSCSBD, Segment_LSDECD):
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
                if m in (Segment_LSCD, Segment_TADDH, Segment_LSCSBD, Segment_LSDECD):
                    args[3] = make_divisible(min(args[3], max_channels) * width, 8)
            if m in (Detect_LSCD, Detect_TADDH, Detect_LSCSBD, Detect_LSDECD):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, 
                              ...
                              Detect_DyHead_Prune, Detect_LSCD, Detect_TADDH, Detect_LADH, Detect_LSCSBD, Detect_LSDECD):
                return "detect"
            elif isinstance(m, (Segment, Segment_Efficient, Segment_LSCD, Segment_TADDH, Segment_LADH, Segment_LSCSBD, Segment_LSDECD)):
                return "segment"
            elif isinstance(m, Classify):
                return "classify"
            elif isinstance(m, (Pose, Pose_LSCD, Pose_TADDH, Pose_LADH, Pose_LSCSBD, Pose_LSDECD)):
                return "pose"
            elif isinstance(m, (OBB, OBB_LSCD, OBB_TADDH, OBB_LADH, OBB_LSCSBD, OBB_LSDECD)):
                return "obb"
            elif isinstance(m, (Detect_NMSFree,)):
                return "nmsfree"
